...
void SAX2CountHandlers::startElement(const XMLCh* const  uri
    ...
    if ((line_num_x = attrs.getValue(tr("line"))) != NULL){
    	...
    	logger.debug(format("Current line number in attr: {}, actual line number {}",line_number, // @suppress("Invalid arguments")
    			    				(unsigned int)flocator->getLineNumber())); // @suppress("Invalid arguments")
    	current_line = line_number;
    }else{
    	logger.error(format("No line number in attr, actual line number {}, file {}",line_number, // @suppress("Invalid arguments")
